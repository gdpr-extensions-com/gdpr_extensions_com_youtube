...
namespace GdprExtensionsCom\GdprExtensionsComYoutube\ViewHelpers;

use GdprExtensionsCom\GdprExtensionsComYoutube\Domain\Repository\GdprManagerRepository;
...
//            dd($gdprManager->_getCleanProperties());
            ...
            if (is_array($gdprManager->_getCleanProperties())) {
                ...
                // Get the extensionTitle and split it to extract 'youtube'
                $extensionTitleParts = explode('-', $properties['extensionTitle']);
                $key = $extensionTitleParts[3];

                // Use 'youtube' as the key in the normalizedGdprManagers array
